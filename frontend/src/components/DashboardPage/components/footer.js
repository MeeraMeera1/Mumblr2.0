    ...
    <div className="footer">
      ...
      {post.likedPost.map((likePost, idx) => {
        if (likePost.userId === sessionUser.id) {
          ...
          return (
            <button
              onClick={likeFeature1}
              className={heart1 ? "like" : "noLike"}
              key={idx}
            >
          ...
          return (
            <button
              onClick={likeFeature2}
              className={heart2 ? "like" : "noLike"}
              key={idx}
            >
              <i className="fas fa-heart" />
            </button>
      ...
      {post.likedPost.length >= 1 &&
        post.likedPost.map((likePost, idx) => {
          if (likePost.userId !== sessionUser.id && buttonCount !== 1) {
            ...
            return (
              <button
                onClick={likeFeature3}
                className={heart3 ? "like" : "noLike"}
                key={idx}
              >
                <i className="fas fa-heart" />
              </button>
      ...
      {!post.likedPost.length && (
        <button onClick={likeFeature4} className={heart4 ? "like" : "noLike"}>
          <i className="fas fa-heart" />
        </button>
      )}
      {showDiv && <div className="like-dropdown">Coming Soon!</div>}
    </div>
  );
};
